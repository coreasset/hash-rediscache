...
@ContextConfiguration(locations = "defaultRedisCacheContext.xml")
//CacheManager에서 캐시를 InMemory에 올려놓아 이를 초기화하여 테스트 하기 위해 컨텍스트를 계속 갱신한다.
@DirtiesContext(classMode = ClassMode.AFTER_EACH_TEST_METHOD)
public class DefaultRedisCacheTest {
...
//		테스트 수행시마다 Redis 캐시내에 데이터를 모두 삭제한다.
	...
	@Test
	public void defaultRedisCacheBugTest() throws Exception {
		
//		다른 캐시명임에도 같은 key를 통해 호출 했을경우 같은 캐시데이터가 반환되는 버그를 검증 
...
//		캐시에서 각각 데이터를 호출했으므로 호출카운트가 증가하지 않는다.
		...
		assertEquals("다른 캐시명, 동일한 캐시Key로 다시 호출하여 실제 메소드가 호출되지 않음(버그!!!)", 
					1, annotationCacheData.cacheData2CallCount());
		
		assertEquals("실제 메소드는 다른 데이터를 반환하나, 다른캐시명/동일한 캐시key일 경우 동일한 캐시데이터가 반환되는 버그로 동일한 데이터가 호출",
